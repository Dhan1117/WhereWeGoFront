...
const WishlistPage = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const normalized = (Array.isArray(data) ? data : data?.items || []).map((p, i) => ({
  ...
  const toggleWishlist = async (touristItem) => {
    // 위시 페이지에서는 ♥ = 삭제
    ...
    try {
      ...
      // 롤백
      setWishlist((prev) => {
        const exists = prev.some((i) => i.id === placeId);
        return exists ? prev : [...prev, touristItem];
      });
    }
  };
      ...
      <Container maxWidth="md" sx={{ pt: 6 }}>
            ...
            {wishlist.map((spot) => (
              <Grid item xs={6} sm={3} key={spot.id}>
                <Card sx={{ position: "relative", borderRadius: 2 }}>
                  <CardMedia component="img" height="140" image={spot.image} alt={spot.name || spot.title} />
        ...
        <Grid container spacing={2} sx={{ mb: 6 }}>
          {recommendedSpots.map((spot) => (
            <Grid item xs={6} sm={3} key={spot.id}>
              <Card sx={{ position: "relative", borderRadius: 2 }}>
                <CardMedia component="img" height="140" image={spot.image} alt={spot.name} />
                <CardContent sx={{ p: 1, pb: "8px !important", textAlign: "center" }}>
                ...
                <IconButton
                  ...
                  disabled
                >
                  {wishlist.some((item) => item.id === spot.id) ? <FavoriteIcon /> : <FavoriteBorderIcon />}
                </IconButton>
              </Card>
            </Grid>
          ))}
          ...
          {planningSpots.map((spot, i) => (
            <Grid item xs={12} sm={6} md={4} key={i}>
              <Card sx={{ borderRadius: 2 }}>
                <CardMedia component="img" height="160" image={spot.image} alt={spot.title} />
              </Card>
            </Grid>
          ))}
        </Grid>
